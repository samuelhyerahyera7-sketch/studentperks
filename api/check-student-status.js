const { clean, json, readBody, requireServiceKey, rest } = require('./_supabase');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await rest(`student_applications?${filter}&order=created_at.desc&limit=1&select=status`);
    const app = rows && rows[0];
    return json(res, 200, { found: !!app, status: app ? app.status : null });
  } catch (error) {
